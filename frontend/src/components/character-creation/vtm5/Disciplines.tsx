...
export default function Disciplines({
  ...
  const [isAlchemy, setIsAlchemy] = useState(false);

  useEffect(() => {
    if (selectedClan === 16) {
      setIsAlchemy(thinbloodAdv.merits.some(adv => adv.Name === "Thinblood Alchemist"));
    }
  }, [selectedClan, thinbloodAdv]);

  const availableDisciplines = disciplineGroups.filter(d => {
    if (selectedClan === 16) {
      return (
        (thinbloodAdv.merits.some(adv => adv.Name === "Discipline Affinity") && d.Name !== "Thin-blood Alchemy") ||
        (isAlchemy && d.Name === "Thinblood Alchemy")
      );
    }
    return disciplineJunction.some(junction => junction.clanID === selectedClan && junction.disciplineID === d.ID);
  });
      ...
      >
        ...
        <div className="mt-4 overflow-y-auto flex-grow no-scrollbar space-y-3 max-h-72">
          {disciplinePowers.map(power => (
            <div
              key={power.ID}
              className="p-3 rounded-lg bg-[var(--color-background-soft)] hover:bg-[var(--color-background-hover)] flex items-center gap-2"
            >
              <input
                type="checkbox"
              ...
              />
              <span className="flex-grow text-[var(--color-foreground)]">
...
function hasLowerLevel(
  ...
  return selectedDisciplines.some(
    d =>
      d.DisciplineGroup === power.DisciplineGroup && d.Level === power.Level - 1
  );
}
