...
export default function ClanBaneSelection({
  ...
  // Memoized computation of selected clan bane to prevent unnecessary re-renders
  const selectedClanBane = useMemo(() => {
    if (selectedClan === null || selectedClan === 15 || selectedClan === 16) {
      return (
        clans.find(
          (clan) =>
            clan.ID !== 15 &&
            clan.ID !== 16 &&
            (clan["Clan Bane"] === selectedBane ||
              clan["Variant Bane"] === selectedBane)
        ) || clans.filter((clan) => {
          if (clan.ID === 15 || clan.ID === 16) return false;
  
          if (selectedClan === 16) {
            const hasBeastialTemper =
              thinbloodAdv?.flaws?.some((flaw) => flaw.Name === "Beastial Temper") ?? false;
            const hasCatenatingBlood =
              thinbloodAdv?.merits?.some((merit) => merit.Name === "Catenating Blood") ?? false;
  ...
  const [selectedBaneType, setSelectedBaneType] = useState<"Standard" | "Variant">(
    selectedClanBane?.["Variant Bane"] === selectedBane ? "Variant" : "Standard"
  );

  const [dropdownOptions, setDropdownOptions] = useState<Clan[]>([]);

  // Memoize dropdown options to prevent unnecessary recalculations
  useEffect(() => {
  ...
  // Ensure selectedBane is valid when selectedClanBane changes
  useEffect(() => {
    if (
  ...
  }, [selectedClanBane, selectedBane, setSelectedBane]);
      ...
      {(selectedClan === 15 || selectedClan === 16) && (
        <div className="w-full flex flex-col items-center">
          <ClanDropdown
            ...
            onSelect={(clan) => {
              setSelectedBane(clan["Clan Bane"]);
            }}
          />
        </div>
      )}
